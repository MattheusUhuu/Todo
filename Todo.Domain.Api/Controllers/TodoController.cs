...
public class TodoController : ControllerBase
{
    ...
    [Route("mark-as-done")]
    [HttpPut]
    public GenericCommandResult MarkAsDone([FromBody] MarkTodoAsDone command, [FromServices] TodoHandler handler)
    {
        command.User = User.Claims.FirstOrDefault(x => x.Type == "user.id")?.Value;
        return (GenericCommandResult)handler.Handle(command);
    }
    ...
    [HttpPut]
    public GenericCommandResult MarkAsUndone([FromBody] MarkTodoAsUndone command, [FromServices] TodoHandler handler)
    {
        command.User = User.Claims.FirstOrDefault(x => x.Type == "user.id")?.Value;
        return (GenericCommandResult)handler.Handle(command);
    }
